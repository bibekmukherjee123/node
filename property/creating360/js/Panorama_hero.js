...
THREE.Panorama_hero = function (dom_element) {
    ...
    this.def_sphere_material = new THREE.MeshBasicMaterial(
        {color: 0x000000});
     ...
     *
     * @param params.preloader_gif {string} : ""
    ...
    this.init = function (params) {
        ...
        if (p.preloader_gif) {
	    var preloaderimg = '/property/property360/public/3d_tour/preloader.gif';
    ...
    this.set_panoramic_texture = function (path, success, error) {
    	...
    	if(path.includes("8080:8080")){path = path.replace("8080:8080","8080");}
    	if(path.includes("/property/property360/public")){console.log("true:",path);
    	}
    	else{
        path = path.replace('http://propertydarshan.com:8080/property/', '/property/');
        //console.log(path);
        path = path.replace('property/', 'property/property360/public/');
...
console.log("activated link");
    ...
    this.from_JSON = function (json) {
        ...
        for (var i = 0; i < data.panoramas.length; ++i) {
            ...
            for (var j = 0; j < item.links.length; ++j) {
                ...
                var imagePath = l.image_path;
		var imageHoverPath = l.image_hover_path;
	...
	console.log(url);
        if(url.includes("http://101.53.152.152/property")){
        url = url.replace('http://101.53.152.152/property', '/property/property360/public');}
        if(url.includes("property/3d_tour")){
        url = url.replace('property/3d_tour','property/property360/public/3d_tour');}
