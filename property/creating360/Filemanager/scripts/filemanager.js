...
var selectItem = function (data) {
	...
	///// --------------------- 3D PANORAMA INTEGRATION --------------------------------------

	// if file manager was opened in IFrame
	if (window.parent !== window) {
		
		window.parent.THREE.VTour_constructor.file_manager.select_file(url);
	}
	
	//$.prompt("Error on select! Check all locations of the script files! ");
...
$(function(){
	...
	// PANORAMA FILE BROWSER CANCEL-BUTTON	
	$('#cancel').click(function() {

		// if file manager was opened in IFrame
		if (window.parent !== window) {
			
			window.parent.THREE.VTour_constructor.file_manager.select_file(undefined);
		}
	});
	...
	if(config.upload.multiple) {
		...
		$('#upload').unbind().click(function() {
			...
			window.dropzone = new Dropzone("div#multiple-uploads", { 
				paramName: "newfile",
				url: fileConnector + '?config=' + userconfig,
